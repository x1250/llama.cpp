...
ggml_tensor * llm_graph_context::build_attn_mha(
    ...
    if (use_flash_attn) {
        ...
        if (kv_idx != nullptr) {
            GGML_ASSERT(n_kv_max > 0 && kv_idx->ne[0] == n_kv_max);
            ggml_flash_attn_ext_set_kv_idx(cur, kv_idx);
        }
        ggml_prec_set_acc(cur, GGML_PREC_F32);
...
ggml_tensor * llm_graph_context::build_attn(
    ...
    ggml_tensor * cur = build_attn_mha(q, k, v, kq_b, kq_mask, sinks, v_mla, 0, nullptr, kq_scale, il);
    cb(cur, "kqv_out", il);
...
ggml_tensor * llm_graph_context::build_attn(
    ...
    ggml_tensor * cur = build_attn_mha(q, k, v, kq_b, kq_mask_top_k, sinks, v_mla, top_k->ne[0], nullptr, kq_scale, il);
    cb(cur, "kqv_out", il);
